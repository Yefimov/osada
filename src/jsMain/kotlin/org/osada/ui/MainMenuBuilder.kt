 ...
 *
 * Buttons keep the ids the action router ([MenuController.mainMenuButton]) toggles (`buy`, `zoom`,
...
internal object MainMenuBuilder {
    ...
    fun buildMainMenu() {
        ...
        // Reparent the three existing status elements (ids kept — MenuController fills them).
        val statusmsg = byId("statusmsg")
        val weathermsg = byId("weathermsg")
        val locmsg = byId("locmsg")
        ...
        statusmsg?.let { statusbar.appendChild(it) }
        // --- weather (text) ---
        weathermsg?.let { statusbar.appendChild(it) }
        ...
        fun iconButton(id: String, action: String, icoMod: String, tip: String) {
        ...
        // --- END TURN (state machine lives in MenuController) ---
        ...
        locmsg?.let { statusbar.appendChild(it) }
