...
internal object Sha256 {
    ...
    fun digest(input: String): String {
        ...
        for (offset in padded.indices step BLOCK_SIZE) processBlock(padded, offset, hash)
        return hash.joinToString("") { it.toUInt().toString(16).padStart(8, '0') }
    }

    private fun pad(input: ByteArray): ByteArray {
        val padding = (BLOCK_SIZE - ((input.size + LENGTH_BYTES + 1) % BLOCK_SIZE)) % BLOCK_SIZE
        val result = ByteArray(input.size + 1 + padding + LENGTH_BYTES)
